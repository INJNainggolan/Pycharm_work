...
def classTest():
    ...
    for i in range(lengthTest):
        ...
        if (classifierResult == numClass):
            ...
            errorCount += 1.0
            print(classifierResult, "!=", numClass)
        ...
        #print("come back result is %s.......real result is %s" %(classifierResult,numClass))
    correctPercent = correctCount/float(lengthTest) * 100.0
    errorPercent = errorCount/float(lengthTest) * 100.0
    print(int(correctCount), lengthTest, int(errorCount), correctPercent, errorPercent)
    file_object.close()
